...
test("renders Kiroku as a private iPhone-first personal memory app", async () => {
  ...
  assert.match(html, /Tu memoria privada, lista para volver a ella/);
  ...
  assert.match(html, /0\.8\.0/);
  assert.match(html, /Apple Watch/);
  assert.match(html, /Ask Kiroku/);
  assert.match(html, /mapas conceptuales/);
  assert.match(html, /grupos privados/);
...
test("renders Kiroku English metadata and product copy", async () => {
  ...
  const html = await response.text();
  assert.match(html, /Kiroku — Horama Apps/);
  assert.match(html, /Your private memory, ready when you return/);
  assert.match(html, /Private alpha · iPhone \+ Watch/);
  assert.match(html, /Multi-device memory/);
  assert.match(html, /Physical and proactive recall/);
  assert.doesNotMatch(html, /\/og\.png/);
});
