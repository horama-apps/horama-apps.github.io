...
export function HomePage({ locale }: { locale: Locale }) {
  ...
  return <main>
    ...
    </div><div className="orbit-card" aria-label={t.home.ecosystem}><div className="orbit-grid" /><div className="orbit-ring ring-one" /><div className="orbit-ring ring-two" /><div className="orbit-center"><img src="/horama-mark.png" alt="Horama" width="115" height="115" /></div>
      {data.apps.map((app, index) => <Link href={`${prefix}/${app.slug}`} className={`orbit-node orbit-node-${index + 1}`} style={{ "--app-color": app.color } as React.CSSProperties} key={app.slug} aria-label={`${t.home.open} ${app.name}`}><AppMark app={app} compact /></Link>)}
