...
export function HomePage({ locale }: { locale: Locale }) {
  ...
  return <main>
    ...
    <section className="hero wrap"><div className="hero-copy">
      ...
      {data.apps.map((app, index) => <Link href={`${prefix}/${app.slug}`} className={`orbit-node orbit-node-${index + 1} orbit-node-${app.slug}`} style={{ "--app-color": app.color } as React.CSSProperties} key={app.slug} aria-label={`${t.home.open} ${app.name}`}><span className="orbit-node-shell"><AppMark app={app} compact /></span><b>{app.name}</b></Link>)}
